  ...
  <div class="column is-8">
    <nav class="level">
      ...
      <div v-if="getUser.isAdmin" class="level-right">
        <div class="level-item">
          <button type="button" v-bind:class="{ 'is-disabled' : this.checkedIds.length <= 0 }" class="button is-danger" @click="remove">Remover</button>
        </div>
        <div class="level-item">
          <button type="button" v-bind:class="{ 'is-disabled' : this.checkedIds.length !== 1 }" class="button is-info" @click="edit">Editar</button>
        </div>
        <div class="level-item">
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="user in userList">
          <th><input type="checkbox" v-model="user.checked"></th>
          <th>{{ user.name }}</th>
          <th>{{ user.access }}</th>
